...
<!-- Your HTML code for header and navigation bar -->
...
<div class="container">
    ...
    <form action="#" method="post">
      ...
      <div class="link-container">
            ...
            <a href="teacher-forgot-password.jsp">Forgot Password?</a>
        </div>  <!-- Additional links or information as needed -->
    </form>
</div>
...
if ("POST".equalsIgnoreCase(request.getMethod())) {
    ...
    try {
        // Query to fetch teacher ID, password, and phone number from the database
        String query = "SELECT teacher_id, password, mobile_no FROM teacher WHERE teacher_id=?";
        ...
        if (rs.next()) {
            // Extract teacher ID
            String teacherIDFromDB = rs.getString("teacher_id");
            // Extract password from the database
            String passwordFromDB = rs.getString("password");
            // Extract phone number from the database
            String phoneNumberFromDB = rs.getString("mobile_no");

            // Check if password is null
            if (passwordFromDB == null) {
                // Extract last four digits of the mobile number
                String lastFourDigits = phoneNumberFromDB.substring(phoneNumberFromDB.length() - 4);
                // Compare last four digits with provided password
                if (userPassword.equals(lastFourDigits)) {
                    // Redirect to teacher dashboard if credentials match
                    response.sendRedirect("teacher-change-password.jsp");
                    return;
                } else {
                    // Display error message in alert popup if credentials don't match
                ...
                if (userPassword.equals(passwordFromDB)) {
                    // Redirect to teacher dashboard if credentials match
                    response.sendRedirect("teacheronly.html");
                    return;
                } else {
                    // Display error message in alert popup if credentials don't match
                    out.println("<script>alert('Invalid credentials');</script>");
            ...
            // Display error message in alert popup if teacher ID is not found
